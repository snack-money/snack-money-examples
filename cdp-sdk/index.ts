...
import { decodeXPaymentResponse, withPaymentInterceptor, type Hex } from "x402-axios";
...
async function main(): Promise<void> {
  ...
  if (!existingWalletAddress) {
    console.error("❌ Missing WALLET_ADDRESS in .env file!");
    console.log("\n💡 Please add your CDP wallet address to the .env file:");
    console.log("   WALLET_ADDRESS=your_wallet_address\n");
    process.exit(1);
  }

  console.log(`✅ Using CDP wallet: ${existingWalletAddress}\n`);
  ...
  const privateKey = await client.evm.exportAccount({
    address: existingWalletAddress as never
  });
